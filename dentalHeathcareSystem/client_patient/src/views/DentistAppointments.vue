  ...
  <main>
        <div class="appointments-container">
          ...
          <div class="appointments-filter">
            ...
            <div class="filter">
              <input id="clinic-name" type="search" v-model="searchQuery" @input="searchAppointments()" placeholder="Search availability, address, clinic">
            </div>
            <div class="filter">
          ...
          <div v-for="appointment in appointments" :key="appointment._id">
            ...
            <div class="appointments-card">
              <div class="appointment-details">
                ...
                <p>Available: {{ appointment.available }}</p>
                <p>Dentist ID: {{ appointment.dentist_id }}</p>
              </div>
...
export default {
  ...
  async mounted() {
    ...
    this.get_filtered_appointments_url = import.meta.env.VITE_FILTER_DENTISTS_APPOINTMENTS_URL;
    this.get_dentists_appointments = import.meta.env.VITE_GET_DENTISTS_APPOINTMENTS_URL;



    await this.getAllClinics();
    await this.getDentistAppointments();
    ...
    methods: {
        ...
        this.error_message = 'Something went wrong failed to retrieve clinic and appointment data. Please try again and reload the page. ';
            setTimeout(() => {
                this.error_message = '';
            }, 5000);
        })
        ...
        async getDentistAppointments() {
            this.loading = true;
        try {
            const response = await Api.get(`${this.get_dentists_appointments}`)
    ...
    async loginCheck() {
      ...
      searchAppointments() {      
        ...
        const options = {
          keys: ['clinic_name', 'clinic_address', 'dentist_id', 'available'],
          threshold: 0.3,
        };
